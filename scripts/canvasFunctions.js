function display(ctx, canvas, text, bg, fg) {
	console.log("Displaying result... ", arguments)
...
function drawWord(ctx, canvas, points, word, color, cover) {
...
function drawBackground(ctx, canvas, color) {
	ctx.fillStyle = color
	ctx.fillRect(0, 0, canvas.width, canvas.height)
}

function drawCounter(ctx, canvas, score, penalty, good, bad) {
...
function drawTotal(ctx, canvas, score, bg, fg) {
	ctx.fillStyle = bg
