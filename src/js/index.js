...
roundCheck = (ctx, canvas) => {
	...
	if (gameState.round >= ROUNDS) {
		gameState.total < TARGET ? display(ctx, canvas, "win", "green", "white") : display(ctx, canvas, "loss", "red", "white");
	};
};
...
getWords().then(words => {
	const drawFrame = now => {
		...
		gameState.roundStartedAt === null ? gameState.roundStartedAt = now : "";
		
		const delta = now - gameState.roundStartedAt;		
		gameState.points = 100 * (1 - delta / (ROUND_DURATION * 1000));
		
		if (Math.floor(gameState.points) <= 0) nextRound();
	...
	document.addEventListener("keypress", (e) => {
		...
		if (key !== BACKSPACE) gameState.text += String.fromCharCode(key).toLowerCase();
	});
	
	document.addEventListener("keydown", (e) => {
		const key = e.which || e.keyCode || e.charCode;
		if (key === BACKSPACE) gameState.text = gameState.text.substr(0, gameState.text.length - 1);

		if (key === ENTER) {
			const [entered, target] = [gameState.text.toLowerCase().trim(), words[gameState.round]];
			entered == target ? nextRound() : (
				gameState.text = "",
